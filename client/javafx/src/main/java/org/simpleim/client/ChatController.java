...
public class ChatController extends Controller {
    ...
    private final ChangeListener<Account> mSelectedItemChangeListener
            = new ChangeListener<Account>() {
        @Override
        public void changed(ObservableValue<? extends Account> observable, Account oldValue,
                Account newValue) {
    ...
    private static String formatChatMessage(String body, User user, long time) {
        return String.format(FORMATTER, time,
                user.getNikename() != null ? user.getNikename() : user.getId(), body);
    }
    ...
    public void setMainApp(MainApp mainApp) {
        ...
        mainApp.getPrimaryStage().getScene().getWindow()
        ...
        KeyCodeCombination keyCombination = new KeyCodeCombination(KeyCode.ENTER,
    ...
    private void initialize() {
        ...
        userList.getSelectionModel().selectedItemProperty()
                .addListener(mSelectedItemChangeListener);
    }
    ...
    private void refreshUserList() {
        userList.getSelectionModel().selectedItemProperty()
                .removeListener(mSelectedItemChangeListener);
    ...
    private final ChatClientListener mChatListener = new ChatClientListenerAdapter() {
        @Override
        public void onReceiveChatMessage(final ChatClientHandler handler,
                final ReceiveMessageNotification message) {
            runInJavaFXApplicationThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (senderId.equals(userList.getSelectionModel().getSelectedItem().getId())) {
                        appendChatLog(message.getMessage().getBody(), message.getSender(),
                                message.getMessage().getSendTime());
                    } else {
                        for (Account account : mUserList) {
                            if (senderId.equals(account.getId())) {
                                ...
                                chatlog.chatLog += formatChatMessage(
                                        message.getMessage().getBody(), message.getSender(),
                                        message.getMessage().getSendTime());
        ...
        @Override
        public void onReceiveLoginNotification(final ChatClientHandler handler,
                final LoginNotification message) {
            runInJavaFXApplicationThread(new Runnable() {
                @Override
                public void run() {
        ...
        @Override
        public void onReceiveLogoutNotification(final ChatClientHandler handler,
                final LogoutNotification message) {
