...
public class ChatClientHandler extends ChannelHandlerAdapter {
    ...
    private static final UpdateFinishedNotification UPDATE_FINISHED_NOTIFICATION
            = new UpdateFinishedNotification();

    private static final LogoutRequest LOGOUT_REQUEST = new LogoutRequest();

    private final Account mAccount = new Account();

    private final CopyOnWriteArrayList<ChatClientListener> mListeners
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ctx.writeAndFlush(
                new LoginRequest().setId(mAccount.getId()).setPassword(mAccount.getPassword()));
    ...
    public static interface ChatClientListener {
        ...
        public void onReceiveLoginNotification(ChatClientHandler handler,
                LoginNotification message);

        public void onReceiveChatMessage(ChatClientHandler handler,
                ReceiveMessageNotification message);

        public void onReceiveLogoutNotification(ChatClientHandler handler,
